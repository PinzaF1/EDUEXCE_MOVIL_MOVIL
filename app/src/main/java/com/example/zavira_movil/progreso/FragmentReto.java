...
public class FragmentReto extends Fragment {
    ...
    private void cargarOponentes() {
        ...
        api.listarOponentes().enqueue(new Callback<List<OpponentBackend>>() { // ✅ Tipo genérico corregido
            @Override
            public void onResponse(Call<List<OpponentBackend>> call, Response<List<OpponentBackend>> resp) {
                if (!isAdded()) return;
            ...
            @Override
            public void onFailure(Call<List<OpponentBackend>> call, Throwable t) {
                if (!isAdded()) return;
    ...
    private void clearChipSelection() {
        chipMat.setSelected(false);
        chipLen.setSelected(false);
        chipCie.setSelected(false);
        chipSoc.setSelected(false);
    ...
    private void crearRetoIrALobby() {
        ...
        api.crearReto(new RetoCreateRequest(25, area, oppId)).enqueue(new Callback<RetoCreadoResponse>() {
            @Override
            public void onResponse(Call<RetoCreadoResponse> call, Response<RetoCreadoResponse> resp) {
                ...
                if (overlay == null) {
                    Toast.makeText(requireContext(), "No existe el contenedor 'container'", Toast.LENGTH_LONG).show();
                    return;
                }
                overlay.setVisibility(View.VISIBLE);

                FragmentLoadingSalaReto f = FragmentLoadingSalaReto
                        .newInstance(creado.getId_reto(), selectedArea, selectedOpponentName != null ? selectedOpponentName : "Oponente");
            ...
            @Override
            public void onFailure(Call<RetoCreadoResponse> call, Throwable t) {
                if (!isAdded()) return;
                btnEnviar.setEnabled(true);
